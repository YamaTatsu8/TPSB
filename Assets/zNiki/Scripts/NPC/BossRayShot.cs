...
public class BossRayShot : MonoBehaviour
{
    // 発射間隔
    ...
    void Start()
    {
        ...
        _fireRate = 1.0f / this.GetComponent<BossWeapon>()._roundsPerSecond;

        _fpsCam = Camera.FindObjectOfType<Camera>();

        _muzzle = this.GetComponent<BossWeapon>().Muzzle;

        _bulletPrefab = this.GetComponent<BossWeapon>()._bulletPrefab;

        _type = this.GetComponent<BossWeapon>()._type;
    }
    ...
    public bool Shot(float fireRate)
    {
        ...
        if (transform.root.GetComponent<EnemyController>())
        {
            _targetPos = transform.root.GetComponent<EnemyController>()._targetPos;
        }
